...
def classify(batch_texts, run_times, tokenizer, model, device, class_mapping, threshold=0.5):
    ...
    for pred in predictions:
        labels = [class_mapping[i] for i, flag in enumerate(pred) if flag]
...
def main(output_dir, pid, bsize):
    ...
    for _ in range(TOTAL_RUNS):
        ...
        _ = classify(batch_texts, run_times, tokenizer, model, device, class_mapping)
    ...
    runtimes_file = os.path.join(
        output_dir,
        f'topicmulti_batch{bsize}_runtime{pid}_tp{throughput:.2f}.csv'
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Benchmark Tweet Topic Multilabel Classification")
